...
def play_board(size):
    ...
    while incomplete_game:
        ...
        if board_full(playing_board, size):
            incomplete_game = not complete(size, playing_board, finished_board)
            if incomplete_game:
                if show_hints == "yes":
    ...
    input("Hit enter to go back to the size selection:")
...
def complete_board(size, board_list):
    ...
    while col_xxx or col_same or not_even_x_o:
        print("LOADING BOARD")
        ...
        while row < size:
            ...
            while sad_life:
                while xxx_ooo:
                    ...
                    #comparing column to see if the rest should be an x or o
                    """
                    if row >= (size - 3):
                        for comparing_column in range(size):
                            tile1counter = 0
                            tile2counter = 0
                            for comparing_row in range(row):
                                if board_list[comparing_row][comparing_column] == TILE1:
                                    tile1counter += 1
                                if board_list[comparing_row][comparing_column] == TILE2:
                                    tile2counter += 1
                            if tile1counter == size // 2:
                                board_list[row][comparing_column] = TILE2
                            elif tile1counter == size // 2:
                                board_list[row][comparing_column] = TILE1
                    """

                    #comparing  row to previous two to check for xxx/ooo in up/down
                    column3 = 0
                    while column3 < size:
                    ...
                    xxx_ooo = xxx_ooo_present(size, row, board_list)
        ...
        col_xxx = col_xxx_ooo(size, board_list)
        if col_xxx:
            row4 = row + 1
        ...
        not_even_x_o = not_even_xo(size, row, row_to_col_list)


        #redos whole board if columns are the same
        col_same = is_col_same(row, row4, size, row_to_col_list, board_list)
    return board_list
#hahahahaha


def xxx_ooo_present(size, row, board_list):
    column = 0
    while column < size - 2:
        if board_list[row][column] == board_list[row][column + 1] == board_list[row][column + 2]:
            while board_list[row][column] == board_list[row][column + 1] == board_list[row][column + 2]:
                random.shuffle(board_list[row])
            return True
        else:
            column += 1
...
def col_xxx_ooo(size, board_list):
    for col in range(size):
...
def starting_board(size, board_list):
    ...
    for row in board_list:
        row[random.randint(1, size - 1)] = EMPTY_TILE

    #storing old list
    old_list = board_list

    while can_remove_tiles < size ** 3:
        remove_number = random.randint(1, 14)
        random.shuffle(random_row_list1)
        random.shuffle(random_row_list2)
        random.shuffle(random_row_list3)

        while can_remove_tiles < size ** 2:
            remove_number = random.randint(1, 9)
            random.shuffle(random_row_list1)
            random.shuffle(random_row_list2)
        ...
        if remove_number == 8 or remove_number == 5 or remove_number == 6 or remove_number == 7:
            pass
        # checks for xx/oo in rows and removes x/o around them
        if remove_number == 10 or remove_number == 9:
            row = random_row_list1[0]
            column = random_row_list3[0]
            if board_list[row][column] == board_list[row][column + 1] != EMPTY_TILE:
                if column == 0:
        ...
        if 3 < remove_number <= 6:
            for row in random_row_list1:
                for column in random_row_list3:
                    if board_list[row][column] == board_list[row][column + 2] != EMPTY_TILE:
                        board_list[row][column + 1] = EMPTY_TILE
                ...
                for column in random_row_list2:
                    if board_list[row][column] == board_list[row][column + 1] != EMPTY_TILE:
                        if column == 0:
                            ...
                            if remove_number == 3 or remove_number == 2 or remove_number == 1:
                                board_list[row][column - 1] = EMPTY_TILE
                                break
                            else:
                                board_list[row][column + 2] = EMPTY_TILE
        ...
        board_list = same(board_list, old_list)
        old_list = board_list
        can_remove_tiles += 1
    ...
    for row in range(size):
        for col in range(size):
            if col < size - 1:
                if board_list[row][col] == board_list[row][col + 1] != EMPTY_TILE:
                    if col == 0:
                        board_list[row][col + 2] = EMPTY_TILE
                    elif col == size - 2:
                        board_list[row][col - 1] = EMPTY_TILE
                    else:
                        board_list[row][col - 1] = EMPTY_TILE
                        board_list[row][col + 2] = EMPTY_TILE
            if row < size - 1:
                if board_list[row][col] == board_list[row + 1][col] != EMPTY_TILE:
                    if row == 0:
                        board_list[row + 2][col] = EMPTY_TILE
                    elif row == size - 2:
                        board_list[row][col - 1] = EMPTY_TILE
                    else:
                        board_list[row - 1][col] = EMPTY_TILE
                        board_list[row + 2][col] = EMPTY_TILE

    for row in range(size):
        for col in range(size):
            board_list[row][col] = board_list[row][col].upper()
    return board_list


def same(new_board, old_board):
    for i, row in enumerate(new_board):
        for i2, row2 in enumerate(new_board):
            if i != i2:
                if row == row2:
                    return old_board
    return new_board

def not_even_xo(size, row, row_to_col_list):
...
def is_col_same(row, row4, size, row_to_col_list, board_list):
    if row4 < row:
        for row in range(size):
            row4 = 0
            while row4 < row:
...
def make_move(size, board):
    ...
    while (not (location.upper() in possible_locations.split(" "))) or location == "":
...
def placing(size, board):
    tile = ""
    while (tile != TILE1) and (tile != TILE2) and (tile != EMPTY_TILE):
...
def complete(size, board, finished_board):
    for row in range(size):
        for col in range(size):
            if board[row][col].lower() != finished_board[row][col].lower():
                return False
    return True
